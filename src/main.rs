...
use std::process::{Command, exit}; // Import Command and exit
...
async fn main() -> Result<()> {
    ...
    // Execute shell command to start spotifyd if it's not running
    Command::new("sh")
        .arg("-c")
        .arg("pgrep spotifyd>/dev/null||/usr/bin/spotifyd")
        .status()
        .expect("failed to execute process");

  panic::set_hook(Box::new(|info| {
       // Execute shell command to kill spotifyd
        Command::new("sh")
            .arg("-c")
            .arg("killall spotifyd")
            .status()
            .expect("failed to execute process");
    panic_hook(info);
  }));
...
async fn start_ui(user_config: UserConfig, app: &Arc<Mutex<App>>) -> Result<()> {
  ...
  loop {
    ...
    // Execute shell command to kill spotifyd before exiting
    Command::new("sh")
        .arg("-c")
        .arg("killall spotifyd")
        .status()
